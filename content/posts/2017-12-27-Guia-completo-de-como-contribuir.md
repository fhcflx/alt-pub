...
- **Apresentação em conferência:** a melhor forma de disponibilizar materiais como pôsteres e diapositivos é depositá-los num serviço especializado como o Slideshare e incluí-los na página da postagem. Caso os autores não desejem esta solução, pode-se incluir uma ligação para baixar o arquivo da apresentação. Os autores devem anexar um "resumo" ou texto explicativo, arbitrariamente conciso, desde que suficiente para explicar do que se trata a apresentação. Também devem indicar em que evento (nome completo, local e data) a apresentação ocorreu. A contribuição de resumos, resumos extendidos e trabalhos completos publicados em anais procede da mesma forma que a pós-publicação de material revisado por pares.
...
- **Texto:** texto simples (`.txt`), microsoft office (`.doc,.docx,.rtf`), open office (`.odt,.fodt`), Markdown (vários "sabores"), html, LaTeX, reStructuredText.
...
- **Vídeo:** qualquer formato, mas apenas será usado `mp4` para visualizar na página. Os outros poderão ser convertidos ou anexados como ligações para baixar os arquivos. O tamanho máximo dos vídeos poderá ser apenas de 20 MB, então recomenda-se que a resolução seja baixa (480p).
- **Audio:** qualquer formato, mas os arquivos ficarão apenas disponíveis para serem baixados. O mesmo limite de tamanho se aplica.
...
- **Bancos de dados:** qualquer formato, mas os arquivos ficarão apenas disponíveis para serem baixados. O mesmo limite de tamanho se aplica.
...
- Se você já tem uma conta no GitHub e já tem noção de como usar Git, faça um _fork_ e clone o projeto, acrescente seus arquivos em formato Markdown na pasta `content/posts` (em português e/ou inglês), suas figuras em `content/images`, seus pdf em `content/docs` e crie um _[pull request][pull]_.
